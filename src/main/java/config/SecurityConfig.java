    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.csrf().disable().authorizeRequests()//
               ...
               /* .antMatchers(Uris.SERVLET_MAP + Uris.VERSION + Uris.TOKENS + "/**").authenticated()//
                ...
                .antMatchers(HttpMethod.GET,Uris.SERVLET_MAP + Uris.VERSION + Uris.STATISTIC + Uris.POPULAR_PRODUCTS)
                .hasAnyRole(Role.ADMIN.name(), Role.MANAGER.name())//
                .antMatchers(Uris.SERVLET_MAP + Uris.VERSION + Uris.STATISTIC + Uris.BY_DATE)
                .hasAnyRole(Role.ADMIN.name(), Role.MANAGER.name())//
                ...
                .antMatchers(Uris.SERVLET_MAP + Uris.VERSION + Uris.GENERATE_PDF + "/**")
                .hasAnyRole(Role.MANAGER.name(), Role.ADMIN.name(), Role.OPERATOR.name())
                .antMatchers(Uris.SERVLET_MAP + Uris.VERSION + Uris.SEED + "/**").hasRole(Role.ADMIN.name())//*/
                //.and().httpBasic();//
    }
